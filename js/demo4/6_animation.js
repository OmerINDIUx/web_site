async function loadAndAnimateSVG() {
  try {
    const response = await fetch("img/collavoration.svg");
    if (!response.ok) throw new Error(`No se pudo cargar el SVG: ${response.status}`);

    const svgText = await response.text();
    const container = document.getElementById('collavoration');
    ...
    cubes.forEach((cube, index) => {
      ...
      let startTime = null;
      const duration = 8000 + Math.random() * 2000; // Cada cubo tiene su duración aleatoria
